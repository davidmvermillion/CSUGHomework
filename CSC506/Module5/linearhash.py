...
class hashTable:
    ...
    def insert(self, record):
        ...
        if self.table[position] == None:
            ...
            print(record.get_name() + " is at position " + str(position))
            isStored = True
            self.elementCount += 1
       
        # collision occured hence we do linear probing
        else:
            print("Collision has occured for " + record.get_name() + str(position) + ". Finding new position.")
    ...
    def search(self, record):
        ...
        if(self.table[position] != None):
            if(self.table[position].get_name() == record.get_name() and self.table[position].get_number() == record.get_number()):
                isFound = True
                print("Record found at position {} ".format(position) + " and total comparisons are " + str(1))
                return position
        ...
        # if element is not found at position returned hash function
                ...
                while self.table[position] != None or self.comparisons <= self.size:
                    ...
                    if(self.table[position].get_name() == record.get_name() and self.table[position].get_number() == record.get_number()):
                        ...
                        print("Record found at position {} ".format(position) + " and total comparisons are " + str(i) )
                        return position
    ...
    def display(self):
        ...
        print("The number of records in the Table are : " + str(self.elementCount))